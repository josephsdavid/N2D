...
model = n2d.n2d(train_x, manifoldLearner=n2d.UmapGMM(n_clusters, umapdim=12, umapN = 20),
                ndim = 20, ae_args={'act':'relu'})
...
for nclust in range(2, 31):
    ...
    mod = n2d.n2d(train_x, manifoldLearner=n2d.UmapGMM(nclust, umapdim=nclust, umapN = 20),
                ndim = nclust, ae_args={'act':'relu'})
...
clust4 = n2d.n2d(train_x,
                 manifoldLearner=n2d.UmapGMM(4, umapdim =4),
                 ndim = 4,
                 ae_args = {'act':'relu'})
...
def grouper(n2d_obj, df):
    ...
    pivot = pd.DataFrame.from_dict(resD)
    return(
        get_cluster(df, pivot, i) for i in np.unique(n2d_obj.preds)
    )

# example frame, just to pick which cluster is most interesting
ef_cl1, ef_cl2, ef_cl3, ef_cl4 = grouper(
    clust4, test_x)
...
pairs = np.column_stack((np.take(idx[0], closest),
                         np.take(idx[1], closest))) + 1
